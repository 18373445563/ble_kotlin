...
class ParaDetailAdapterTest(var paraList: ArrayList<ParaEntity>, var ctx: Context) :
    RecyclerView.Adapter<ParaDetailAdapterTest.ViewHolder>() {
    //参数显示对话框
    ...
    fun showParamDialog(paraDate: ParaEntity, type: Int) {
        try {
            ...
            with(view) {
                ...
                //设置值
                //部分数据加上单位等
                if (paraDate.accuracy.equals("")) {
                    param_valueTxt.setText(paraDate.realValue + paraDate.unit)
                } else {
                    var data1 = paraDate.realValue?.toFloat()
                    var data2 = paraDate.accuracy?.toFloat()
                    if (data1 != null && data2 != null) {
                        param_valueTxt.setText((data1 / data2).toString() + paraDate.unit)
                    }
                }
                    ...
                    if (BLEManager.isConnected) {
                        ...
                        GlobalScope.launch(Dispatchers.Main) {
                            ...
                            if (DetailParaActivity.isSuccess) {
                                ...
                                var realData = paramRealValue.get(
                                    para_address.text.toString().toInt()
                                )!!.realValue
                                //param_valueTxt.setText()
                                //部分数据加上单位等
                                if (paraDate.accuracy.equals("")) {
                                    param_valueTxt.setText(realData + paraDate.unit)
                                } else {
                                    var data1 = realData?.toFloat()
                                    var data2 = paraDate.accuracy?.toFloat()
                                    if (data1 != null && data2 != null) {
                                        param_valueTxt.setText((data1 / data2).toString() + paraDate.unit)
                                    }
                ...
                param_writeBtn.setOnClickListener(View.OnClickListener {
                    ...
                    var inputData = param_writeEdit.text.toString().replace(".", "").toInt()
                    if (inputData < rangeList[0].toFloat() || inputData > rangeList[1].toFloat()) {
                        ...
                        if (BLEManager.isConnected) {
                            BLEManager.scopeSetDeal.launch {
                                ...
                                launch(Dispatchers.Main) {
                                    ...
                                    if (!currentData.equals(param_writeEdit.text.toString())) {
                                ...
                                launch(Dispatchers.Main) {
                                    ...
                                    if (!currentData.equals(param_value_radio.text.toString())) {
                                        ...
                                        setLogOp(
                                            ...
                                            BLEManager.currentBleName,
                                            "2",
                                            param_value_radio.text.toString(),
                                            "0"
                                        )
                                    } else {
                                        param_valueTxt.setText(
